"""TCP Server for Hardware Track Controller.

Listens for wayside status messages from the Raspberry Pi and forwards
them to CTC backends running on the laptop.
"""
...
class HardwareTrackControllerServer:
    """TCP server that receives wayside status updates from Raspberry Pi.
            ...
            while self._running:
                ...
                while b"\n" in buffer:
                    ...
                    try:
                        ...
                        logger.exception(
                            "[HWTC-Server] Failed to decode JSON from %s:%d", *addr
                        )
                        continue
    ...
    def _handle_wayside_status(self, msg: dict[str, Any]) -> None:
        """Apply wayside status updates to the correct CTC backend."""
        ...
        for u in updates:
            ...
            if "occupied" in u and hasattr(ctc_backend, "update_block_occupancy"):
                try:
                    ctc_backend.update_block_occupancy(
                        line_name, block_id, bool(u["occupied"])
                    )
                except Exception:
            ...
            if "signal_state" in u and hasattr(ctc_backend, "update_signal_state"):
                try:
                    ctc_backend.update_signal_state(
                        line_name, block_id, u["signal_state"]
                    )
                except Exception:
            ...
            if "switch_position" in u and u["switch_position"] is not None:
                if hasattr(ctc_backend, "update_switch_position"):
                    try:
                        ctc_backend.update_switch_position(
                            line_name, block_id, u["switch_position"]
                        )
                    except Exception:
            ...
            if "crossing_status" in u and u["crossing_status"] is not None:
                if hasattr(ctc_backend, "update_crossing_status"):
                    try:
                        ctc_backend.update_crossing_status(
                            line_name, block_id, bool(u["crossing_status"])
                        )
                    except Exception:
    ...
    def _call_ctc_method(
        ...
        """Call CTC method with appropriate signature."""
