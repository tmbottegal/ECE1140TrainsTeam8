...
class FailuresShit:
    ...
    def _detect_power_failure(self, block_id: int, command_type: str, expected_value: any) -> None:
        ...
        if verification_key not in self._command_retry_count: self._command_retry_count[verification_key] = 0
    ...
    def _check_train_movement(self, block_id: int) -> bool:
        if block_id not in self._occupancy_changes: return False
        if self._known_occupancy.get(block_id, False): return False  
    ...
    def _handle_broken_rail(self, block_id: int) -> None:
        ...
        except Exception as e: logger.error(f"Signals no set for broken rail: {e}")
        try:
        ...
        except Exception as e: logger.error(f"Trains no stop for broken rail: {e}")
    ...
    def _handle_track_circuit_failure(self, block_id: int) -> None:
        ...
        try:
            for nearby_block in range(max(1, block_id-2), min(block_id+3, self.num_blocks+1)):
                if nearby_block in self._line_block_ids(): self.set_signal(nearby_block, SignalState.RED)
        except Exception as e: logger.error(f"Failed to set the damn signals for track circuit failure: {e}")
    
    def _get_adjacent_blocks(self, block_id: int) -> list[int]:
        adjacent = []
        if block_id > 1: adjacent.append(block_id - 1)
        if block_id < self.num_blocks: adjacent.append(block_id + 1)
    ...
    def _apply_dynamic_plc_logic(self, plc_module) -> None:
        ...
        for block_id in self._line_block_ids():
            try:
                ...
                if base_speed is None or base_auth is None: continue
                ...
                if get_speed_for_signal:
                    ...
                    if signal_str == "RED": adjusted_speed = 0
                    elif signal_str == "YELLOW": adjusted_speed = int(base_speed * yellow_factor)
                    elif signal_str == "SUPERGREEN": adjusted_speed = min(base_speed, speed_limit)
                if get_authority_for_signal:
                    adjusted_auth = get_authority_for_signal(base_auth, signal_str, occupied)
                else:
                    if signal_str == "RED": adjusted_auth = 0
                    elif occupied: adjusted_auth = max(int(base_auth * 0.5), 50)
                if adjusted_speed == 0: new_signal = SignalState.RED
                elif adjusted_speed < base_speed * 0.5: new_signal = SignalState.YELLOW
                elif adjusted_speed >= speed_limit: new_signal = SignalState.SUPERGREEN
                else: new_signal = SignalState.GREEN
...
class TrackControllerBackend(FailuresShit):
    ...
    def _send_status_to_ctc(self) -> None:
        ...
        for block_id in self._line_block_ids():
            ...
            switch_pos = None
            if block_id in self.switches: switch_pos = self.switches[block_id]
        ...
        try:
            if hasattr(self.ctc_backend, 'receive_wayside_status'):
                ...
                for status in status_updates: self._send_single_status_to_ctc(status)
            ...
            if isinstance(status, dict) and "segments" in status:
                ...
                for block_id, info in segments.items():
                    ...
                    if "occupied" in info:
                        ...
                        self._previous_occupancies[bid] = bool(info["occupied"])
                        self._previous_occupancy[bid] = bool(info["occupied"])
                ...
                for block_id in self._line_block_ids():
                    if block_id in self.track_model.segments:
                        ...
                        if hasattr(seg, 'occupied'):
                            self._known_occupancy[block_id] = bool(seg.occupied)
                            self._previous_occupancies[block_id] = bool(seg.occupied)
                            self._previous_occupancy[block_id] = bool(seg.occupied)
                            logger.debug("Direct sync block %d occupancy: %s", block_id, seg.occupied)
                        if hasattr(seg, 'signal_state'):
                            self._known_signal[block_id] = seg.signal_state
                            logger.debug("Direct sync block %d signal: %s", block_id, seg.signal_state)
    ...
    def _upload_plc_python(self, filepath: str) -> None:
        ...
        else: logger.error("PLC module missing plc_logic function")

    def _execute_plc_logic(self) -> None:
        if self._plc_module is None or not hasattr(self._plc_module, 'plc_logic'):
        ...
        try:
            ...
            switch_positions, light_signals, crossing_signals, stop = self._plc_module.plc_logic(
                block_occupancies, 
                switch_positions, 
                light_signals, 
                crossing_signals, 
    ...
    def _sync_after_plc_upload(self) -> None:
        for block_id in self._line_block_ids():
            ...
            if not seg: continue
            if hasattr(seg, 'signal_state'): self._known_signal[block_id] = seg.signal_state
    ...
    def report_state(self) -> Dict[str, object]: # im not fixing how ugly this looks, sue me
        switches_display = {}
        for sid, pos in self.switches.items(): switches_display[sid] = "Normal" if pos == 0 else "Alternate"
        crossings_display = {}
        for cid, status in self.crossings.items():
            crossings_display[cid] = { "block": self.crossing_blocks.get(cid), "status": "Active" if status else "Inactive"}
        return {"line": self.line_name, "maintenance_mode": self.maintenance_mode, "blocks":
                 {b: {"occupied": d["occupied"], "suggested_speed": d["suggested_speed"], "suggested_auth": d["suggested_auth"],
                    "commanded_speed": d["commanded_speed"], "commanded_auth": d["commanded_auth"], "signal": (d["signal"].name.title() 
    ...
    def _update_occupancy_from_model(self, block_id: int, occupied: bool) -> None:
        ...
        if old_state != occupied and self._plc_module is not None:
            logger.info("Occupancy changed for block %d, re-executing PLC logic", block_id)
            self._execute_plc_logic()
        for cid, cblock in self.crossing_blocks.items():
            if cblock == block_id:
                try:
                ...
                except Exception: logger.exception("Failed to auto-update crossing gate for block %d", block_id)
        if occupied:
                ...
                if isinstance(segment, type(segment)) and hasattr(segment, 'straight_signal_state'):
                    if hasattr(segment, 'previous_signal_state'):
                        ...
                        if known_prev != current_prev:
                            ...
                            logger.info("Switch %d previous signal updated from model: %s", block_id, current_prev)
                            state_changed = True
                    if hasattr(segment, 'straight_signal_state'):
                        ...
                        if known_straight != current_straight:
                            ...
                            logger.info("Switch %d straight signal updated from model: %s", block_id, current_straight)
                            state_changed = True
                    if hasattr(segment, 'diverging_signal_state'):
                        ...
                        if known_div != current_div:
                            ...
                            logger.info("Switch %d diverging signal updated from model: %s", block_id, current_div)
                            state_changed = True
                else:
    ...
    def _line_block_ids(self) -> List[int]:
        ...
        if rng is None: return sorted(self.track_model.segments.keys())
    ...
    def remove_listener(self, callback: Callable[[], None]) -> None:
        ...
        except ValueError: logger.debug("Listener %r not registered for %s", callback, self.line_name)

    def _notify_listeners(self) -> None:
        for cb in list(self._listeners):
            try: cb()
            except Exception: logger.exception("Listener %r raised exception while notifying", cb)
        ...
        for b in self._line_block_ids():
            ...
            try:
                ...
                if b in self.switches:
                    signal_val = {
                        'previous': self._switch_signals.get((b, 0), "N/A"),
    ...
    def set_signal(self, block: int, color: str | SignalState, signal_side: int = 0) -> None:
        if isinstance(color, SignalState): state = color
        else:
            try:
                enum_name = str(color).replace(" ", "").upper()
                state = SignalState[enum_name]
            except Exception: raise ValueError(f"Invalid signal color '{color}'")
        if block in self.switches:
            if not hasattr(self, '_switch_signals'): self._switch_signals = {}
            self._switch_signals[(block, signal_side)] = state
        else: self._known_signal[block] = state
        try:
            self.track_model.set_signal_state(block, signal_side, state)
            logger.info("Sent to Track Model: Block %d signal (side %d) -> %s", block, signal_side, state.name)
        except Exception as klee: logger.warning("Failed to set signal %d (side %d) in Track Model: %s", block, signal_side, klee)
        self._notify_listeners()
        self._send_status_to_ctc()
    ...
    def safe_set_switch(self, switch_id: int, position: int | str) -> None:
        ...
        if isinstance(position, str):
            ...
            if pos_str == "Normal": pos_int = 0
            elif pos_str == "Alternate": pos_int = 1
            else: raise ValueError("Invalid switch position")
        else:
        ...
        except Exception as e: logger.warning("Failed to set switch %d in Track Model: %s", switch_id, e)
        self._notify_listeners()
        self._send_status_to_ctc()
    ...
    def safe_set_crossing(self, crossing_id: int, status: bool | str) -> None:
        ...
        if isinstance(status, str):
            ...
            if stat_str == "Active": stat_bool = True
            elif stat_str == "Inactive": stat_bool = False
            else: raise ValueError("Invalid crossing status")
        else: stat_bool = bool(status)
        ...
        if block:
            try:
                ...
                if hasattr(seg, 'set_gate_status'):
                    ...
                    logger.info("Track Model: Crossing %d (block %d) gate status set to: %s", crossing_id, block, stat_bool)
                else:
                    logger.warning("Block %d does not have set_gate_status method", block)
            except Exception as e: logger.warning("Failed to set crossing %d in track model: %s", crossing_id, e)
        self._notify_listeners()
        self._send_status_to_ctc()
