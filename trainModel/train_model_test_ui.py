"""Train Model Test UI - Train Controller testbench interface.
"""
...
class TrainModelTestUI(QWidget):
    """Testbench UI for train controller integration testing.
    
    Attributes:
        backend: TrainModelBackend instance for state management.
        power_spin: QDoubleSpinBox for power display.
        cmd_speed_spin: QDoubleSpinBox for commanded speed input.
        authority_spin: QDoubleSpinBox for authority display.
    """
    ...
    def __init__(self, backend: "TrainModelBackend") -> None:
        """Initialize train model test UI.
        
        Args:
            backend: TrainModelBackend instance to control.
        """
        super().__init__()
        ...
        self.authority_spin.setButtonSymbols(
        ...
        self.current_temp_label.setButtonSymbols(
            QDoubleSpinBox.ButtonSymbols.NoButtons
        )
        ...
        cabin_label.setAlignment(
            Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter
        )
        grid.addWidget(cabin_label, 1, 0)
        
        self.cabin_lights_btns = self._create_toggle_buttons(
            "Off", "On", "cabin_lights"
        )
        ...
        head_label.setAlignment(
            Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter
        )
        grid.addWidget(head_label, 2, 0)
        
        self.headlights_btns = self._create_toggle_buttons(
            "Off", "On", "headlights"
        )
        ...
        self.left_doors_btns = self._create_toggle_buttons(
            "Closed", "Open", "left_doors"
        )
        ...
        self.right_doors_btns = self._create_toggle_buttons(
            "Closed", "Open", "right_doors"
        )
    ...
    def _on_temp_changed(self, value: float) -> None:
        ...
        Args:
            value: New temperature setpoint in Fahrenheit.
        """
        if self._is_syncing:
            return
        try:
    ...
    def _generate_announcement_from_beacon(self, beacon: str) -> None:
        """Generate station announcement from beacon data.
        
        Args:
            beacon: Beacon data string.
        """
    ...
    def _sync_from_backend(self) -> None:
        """Synchronize UI controls from backend state."""
        if self._is_syncing:
            return
        try:
            ...
            logger.exception(
                "Failed to sync testbench from backend: %s",
                e
            )
        finally:
            self._is_syncing = False
    ...
    def _refresh_display(self) -> None:
        """Refresh display elements from backend state."""
        if self._is_syncing:
            return
        try:
            self._is_syncing = True
            ...
            if temp_setpoint_c is not None:
                temp_setpoint_f = (
                    float(temp_setpoint_c) * 9.0 / 5.0 + 32.0
                )
            ...
            self._set_toggle_state(
                "service_brake",
                bool(s.get("service_brake", False))
            )
            self._set_toggle_state(
                "emergency_brake",
                bool(s.get("emergency_brake", False))
            )
            self._set_toggle_state(
                "cabin_lights",
                bool(s.get("cabin_lights", False))
            )
            self._set_toggle_state(
                "headlights",
                bool(s.get("headlights", False))
            )
            self._set_toggle_state(
                "left_doors",
                bool(s.get("left_doors", False))
            )
            self._set_toggle_state(
                "right_doors",
                bool(s.get("right_doors", False))
            )
            self._set_toggle_state(
                "engine_failure",
                bool(s.get("engine_failure", False))
            )
            self._set_toggle_state(
                "brake_failure",
                bool(s.get("brake_failure", False))
            )
            self._set_toggle_state(
                "signal_pickup_failure",
                bool(s.get("signal_pickup_failure", False))
            )
    ...
    def _controller_step(self) -> None:
        ...
        try:
            ...
            v_cmd_driver_mps = (
                float(self.cmd_speed_spin.value()) / self.MPS_TO_MPH
            )
            v_cmd = (
                v_cmd_driver_mps if v_cmd_driver_mps > 0.0
                else v_cmd_backend
            )
            
            self.backend.set_inputs(
                commanded_speed_mph=v_cmd * self.MPS_TO_MPH
            )
            ...
            if (self.service_brake_btns[1].isChecked() or
                self.emergency_brake_btns[1].isChecked()):
                power_cmd_kw = 0.0
    ...
    def _create_toggle_buttons(
        ...
        Args:
            off_text: Label for off button.
            on_text: Label for on button.
            attr_name: Backend attribute name to control.
            red: Use red styling for 'on' state if True.
            green: Use green styling for 'off' state if True.
        ...
        def _apply_to_backend(enabled: bool) -> None:
            ...
            try:
                ...
                if attr_name in failure_types:
                    mapping = {
                        "engine_failure": "engine",
                        "brake_failure": "brake",
                        "signal_pickup_failure": "signal"
                    }
                ...
                logger.exception(
                    "Failed to push toggle '%s' to backend: %s",
                    attr_name,
                    e
                )
        
        def update_style_and_push() -> None:
            """Update button styling and push state to backend."""
            if self._is_syncing:
                return
            ...
            if off_btn.isChecked():
                ...
                off_btn.setStyleSheet(
                    f"background:{off_color}; color:#111; "
                    f"font-weight:bold; padding:6px;"
                )
                on_btn.setStyleSheet(
                    "background:#2a2a2a; color:#888; padding:6px;"
                )
            else:
                off_btn.setStyleSheet(
                    "background:#2a2a2a; color:#888; padding:6px;"
                ...
                on_btn.setStyleSheet(
                    f"background:{on_color}; color:#111; "
                    f"font-weight:bold; padding:6px;"
                )
    ...
    def _set_toggle_state(self, attr_name: str, enabled: bool) -> None:
        """Set toggle button state without triggering callbacks.
        
        Args:
            attr_name: Attribute name of toggle to update.
    ...
    def _play_announcement(self) -> None:
        ...
        if announcement_text:
            ...
            QMessageBox.warning(
                self,
                "No Announcement",
                "Please enter an announcement first."
            )